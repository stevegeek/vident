...
- **`has_stimulus_controller` class method** (#27). Re-enables the implied controller in a subclass when the parent declared `no_stimulus_controller`. Lets a generic shell/base component opt out while specific subclasses with paired JS controllers opt back in without reaching into private ivars.
- **`root_element_class_list(extra_classes = nil)` and `root_element_data_attributes`** (#25). Public instance methods on every Vident component returning the composed root-element `class=` string and `data-*=` Hash — for components that render through third-party helpers (e.g. `inline_svg_tag`) instead of `root_element(...)`. V1's `render_classes` / `stimulus_data_attributes` renamed for V2 consistency; see UPGRADING.md §8.
- **Class-level Stimulus builders** (#18). `MyComponent.stimulus_target(:x)`, `.stimulus_action(:click, :save)`, `.stimulus_value(:n, 0)`, etc., return `Vident::Stimulus::*` value instances parallel to the instance-level methods. Useful for Turbo-Stream partials, JSON responses, and test selectors that need the Stimulus attribute object without a component instance. `stimulus_outlet` at class level requires an explicit selector (no `component_id` to auto-scope); cross-controller forms are rejected.
- **`Vident::Types::*` Literal type aliases** (#16). Seven named unions for the built-in `stimulus_*:` prop types — `StimulusControllers`, `StimulusActions`, `StimulusTargets`, `StimulusOutlets`, `StimulusValues`, `StimulusParams`, `StimulusClasses`. The same objects back the built-in props in `Declarable`; re-use them in user components (`prop :extra_actions, Vident::Types::StimulusActions`).
- **`cache_component(*extra_keys, **options, &block)` on both adapter base classes** (#13). Wraps render output with Rails.cache using the Vident-computed `cache_key`. On Phlex: delegates to `Phlex::SGML#cache` (block-scoped fragment cache inside `view_template`). On ViewComponent: uses `Rails.cache.fetch` + `capture` (works inside a `def call` method; sidecar ERB templates can use Rails' own `<% cache cache_key do %>` helper). Raises `Vident::ConfigurationError` if the component has no `with_cache_key` declared. Key computation (`component.cache_key`) and fragment wrapping are separable — the key is still available on its own for etags or manual `Rails.cache.fetch` at the call site.
