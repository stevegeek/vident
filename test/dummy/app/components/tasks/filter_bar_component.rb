...
module Tasks
  class FilterBarComponent < ApplicationComponent
    STATUSES = %i[all todo done wont_do].freeze
      ...
      root_element(class: "flex flex-wrap items-center gap-3 rounded-lg bg-gray-50 p-3") do |bar|
        ...
        bar.child_element(
          ...
          STATUSES.each do |status|
            # Phlex auto-kebab-cases symbol attribute values (`:wont_do` → `"wont-do"`).
            # Stringify so the option value matches `status` straight back through.
            option(value: status.to_s, selected: (status == @active_filter)) { status.to_s.tr("_", " ").capitalize }
          end
        end
