...
module Dashboard
  class FilterBarComponent < ApplicationComponent
    ...
    stimulus do
      # `values_from_props` mirrors the :active_filter prop into a Stimulus value
      # without a second declaration. The JS controller reads it as
      # `this.activeFilterValue`.
      values_from_props :active_filter

      # filter_select / search_input are wired onto their specific child_elements
      # below (event prefixes `[:change, ...]` / `[:input, ...]`). The root only
      # needs the reverse-channel listener: the page dispatches filterApplied
      # with the visible count so we can repaint the badge.
    ...
    def view_template
      root_element(class: "flex flex-wrap items-center gap-3 rounded-lg bg-gray-50 p-3") do |bar|
        ...
        # `child_element` builds a tag with stimulus_* kwargs resolved into data
        # attributes. `[:change, :filter_select]` means "on change event, fire
        # the filter_select action on this controller".
        bar.child_element(
          :select,
