...
class DashboardReleaseCardComponentTest < Minitest::Test
  def build(overrides = {})
    defaults = {release_id: 42, name: "Widget", version: "1.2.3", environment: :production, status: :deployed}
    Dashboard::ReleaseCardComponent.new(**defaults.merge(overrides))
  end

  def test_stimulus_identifier_matches_v1
    assert_equal "dashboard/release_card_component",
      Dashboard::ReleaseCardComponent.stimulus_identifier_path
    assert_equal "dashboard--release-card-component",
      Dashboard::ReleaseCardComponent.stimulus_identifier
  end
