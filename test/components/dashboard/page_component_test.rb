...
class DashboardPageComponentTest < Minitest::Test
  RELEASES = [
  ...
  def build(overrides = {})
    Dashboard::PageComponent.new(releases: RELEASES, **overrides)
  end

  def test_stimulus_identifier_matches_v1
    assert_equal "dashboard/page_component",
      Dashboard::PageComponent.stimulus_identifier_path
    assert_equal "dashboard--page-component",
      Dashboard::PageComponent.stimulus_identifier
  end
  ...
  def test_renders_with_empty_releases
    html = Dashboard::PageComponent.new.call
    assert_includes html, 'data-controller="dashboard--page-component"'
