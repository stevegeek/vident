...
class PhlexAvatarComponentTest < Minitest::Test
  ...
  def test_image_avatar_predicate_method
    ...
    # Should return false when URL is empty string
  ...
  def test_shape_class_method
    circle_component = Phlex::AvatarComponent.new(initials: "JD", shape: :circle)
    assert_equal "rounded-full", circle_component.send(:shape_class)

    square_component = Phlex::AvatarComponent.new(initials: "JD", shape: :square)
  ...
  def test_size_classes_method
    ...
    test_cases.each do |size, expected_classes|
      component = Phlex::AvatarComponent.new(initials: "JD", size: size)
  ...
  def test_inheritance_structure
    # Test that the component inherits from some form of ApplicationComponent
    # The component is defined within the Phlex module and inherits from ApplicationComponent
    assert Phlex::AvatarComponent.superclass.name.include?("ApplicationComponent")
  end
  ...
  def test_image_avatar_vs_text_avatar_element_tag
    # Image avatar should use img tag
    image_component = Phlex::AvatarComponent.new(
      initials: "JD",
      url: "https://example.com/avatar.jpg"
    )
    image_attrs = image_component.send(:root_element_attributes)
    assert_equal :img, image_attrs[:element_tag]

    # Text avatar should use div tag
    text_component = Phlex::AvatarComponent.new(initials: "JD")
    text_attrs = text_component.send(:root_element_attributes)
